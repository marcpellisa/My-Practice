...
def round_scores(student_scores):
    ...
    for score in student_scores:
        rounded_values.append(round(score))
...
def letter_grades(highest):
    ...
    lower = 40
    Dmin = lower + 1
    Cmin = int((highest-lower)/4 + lower +1)
    Bmin = int(2*(highest-lower)/4 + lower +1)
    Amin = int(3*(highest-lower)/4 + lower +1)
    return [Dmin,Cmin,Bmin,Amin]
...
def student_ranking(student_scores, student_names):
    ...
    for i in range(len(student_scores)):
        ranking.append(str(i+1)+'. '+student_names[i]+': '+str(student_scores[i]))
    return ranking
